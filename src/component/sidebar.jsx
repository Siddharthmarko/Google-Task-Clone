...
export default function SideBar(){
    ...
    <div className="flex" >
                ...
                <div onClick={() => dispatcher({ type: 'all' })} className="flex items-center  hover:bg-indigo-50  p-4 text-center w-[220px] h-[34px] rounded-[15px]">
                    <CheckOutlined />
                        ...
                        {state.map((item) => {
                            return (
                                <li className="flex px-4 gap-9 hover:bg-indigo-50" key={item.id}>
                                    <input type="checkbox" name={item.name} checked={item.checked} onChange={(e) => dispatcher({
                                        type: 'check', id:
                                            item.id, value: e.target.checked
                                    })}
                                    />
